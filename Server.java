...
public class Server {
    ...
    String[] listOfServers = {"ServerPart1","ServerPart2","ServerPart3"};
    ...
    public void send() throws Exception {
        int part = 3;   //nbr de sous-server
        int offset = 0; //depart du lecture dans chaque sous-server
        int choice = getClient().getInputStream().readInt();    //choix de menu 1: send 2: download 3: exit
        if(choice ==1) {
            ...
            Socket socket = null;
            for (int i = 1; i <= part; i++, offset += portion) {    //partage le fichier sur les 3 sous-server
            ...
            for (int i = 1; i <= part; i++) {
                Socket socket = new Socket("localhost", 5000 + i);
                out = new DataOutputStream(socket.getOutputStream());
                out.writeInt(choice);
                out.writeInt(file.getBytes().length);
                out.write(file.getBytes());
...
//envoie au serveur principale le folder
    ...
    public static void getFile(InputStream inputStream, String folder) throws Exception {
        try {
            String pathDownload = "D:\\Bossy\\L2\\S3\\ProgrammationSysteme Mr Naina\\FileTransfertSocket\\DownloadFile\\";
            int offset = 0;
            int lenght = 0;
            String fileName = getFileName(inputStream);
